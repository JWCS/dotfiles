...
" Reload :e :edit :e! (discard), set autoread?
...
" Replaced with farmergreg/vim-lastplace
...
fun! DockerfileFoldExpr(lnum)
  ...
  if content =~ '^\s*$' | return '-1' | endif " Blanks
  if content =~ '^FROM' | return '0' | endif " FROM
  if content =~ '^##' | return '0' | endif " Comment
  if content =~ '^#' | return '1' | endif " Comment
  if content =~ '^\u+' | return '>1' | endif " ALLCAP Directive
  "if content =~ '^\s\+\S' | return '3' | endif " Indent
  return '2' " Normal sections
endfun
...
if !(empty($VRC_FEAT_TP_STD))
  au Filetype sh nmap <silent> <leader>' ysiW"
endif

au FileType dockerfile setl ts=2 sw=2 sts=2 foldmethod=expr foldlevel=2
  \ foldexpr=DockerfileFoldExpr(v:lnum)
